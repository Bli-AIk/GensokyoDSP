...
impl MorphOscillator {
    ...
    fn morph_waveform(phase: f64, form: f64) -> f64 {
        ...
        if form < 0.57 {
            ...
            // 修正偶次/奇次比例：从1.0 (锯齿波) 逐渐降低到 0.0 (方波)
            // 0.57->1.0, 0.6->0.85, 0.65->0.65, 0.7->0.45, 0.75->0.20, 0.8->0.00
            ...
            // 根据实测数据：占空比从50.0% (0.8) 到 99.0% (1.0)
            // 稍微降低最大占空比以减少刺耳感 (0.995 -> 0.985)
            ...
            // 减少极端情况下的谐波数量和rolloff强度，以减少刺耳感
            let max_harmonics = if duty_cycle > 0.97 {
                512 // 降低最大谐波数量 1024 -> 512
